<?php
	...
	else if (isset($_POST['usave']))
	{
		...
		else
		{
			...
			$sql = mysqli_query($db,"select * from User where password='$old' and login='$email'");
			if(mysqli_num_rows($sql) > 0)
			{
				if(!empty($npass) && !empty($cpass))
				{
					if($npass == $cpass)
					{
						mysqli_query($db,"update User set password='$npass' where password='$old' and login='$email'");
	...
	else if(isset($_POST['submit']))
	{
		...
		{
			$sql = mysqli_query($db,"select * from User where login='$Username' AND password='$Password' ");
	...
	function user()
	{
		...
		if(mysqli_num_rows($sql) > 0)
		{
			...
			mysqli_query($db,"update 
				...
				set LastLogin='$datetime' where Email='$email'");
